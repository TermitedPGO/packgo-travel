...
## v691 重驗（三項 bug fix 驗收）

**重驗日期：** 2026-06-11  
**bundle 確認：** console 錯誤仍顯示 `index-CgsaKXz2.js`（v690 hash 未變）→ **v691 尚未部署**  
**方法：** 導覽至 /workspace → 供應商監控 → AI 對話 → 行銷海報詳情，逐項視覺 + console 確認

---

### 重驗 1 — 毛利卡（P1 marginAudit SQL）

| 檢查點 | 結果 |
|--------|------|
| console 有無 marginAudit 500 errors | ❌ 仍有 — 讀取到 18+ 個 `CASE-in-JOIN` TRPCClientError（timestamps 5:17–5:38 PM）|
| 供應商 → 監控 UI banner | ❌ 仍顯示「**毛利查詢暫時失敗，稍後再試（不影響其他資料）**」灰色 banner |
| 毛利卡數字 | ❌ 空白 — SQL 仍未修復，無法返回資料 |

**判定：❌ 未修復**（v691 未部署，SQL 仍是舊版 CASE-in-JOIN 模式）
...
| 檢查點 | 結果 |
|--------|------|
| 進入「與 AI 對話」頁面 | ✅ 直接顯示 06/11 18:35-36 歷史對話（Q2 本月淨利 -$512 完整可見）|
| 切去別頁（今日待辦）再切回 | ✅ 對話歷史完全保留，未重置 |
| 之前的 Q2 回答內容 | ✅「這個月（6月）淨利是 -$512，虧損中。收入：$3,835 支出：$4,347...」完整顯示 |

**判定：✅ 已修復**（對話歷史跨 navigate 正確持久化）

---

### 重驗 3 — Newsletter 平台文案 raw JSON（P2）

| 檢查點 | 結果 |
|--------|------|
| 行銷 → 海報 → 開「待審核」海報詳情 Sheet | 可打開 |
| 電子報平台欄位顯示 | ❌ 仍顯示完整 JSON：`{"platform": "Email Newsletter", "content": {"subject_line": "【限時揭秘】北京-西安深度體驗...`, 換行+巢狀 JSON 完整展開 |
| 應顯示 | 渲染後的 subject + body 文字 + hashtag badges |

**判定：❌ 未修復**（電子報欄位 JSON 未 parse + render）
...
| Bug | 優先 | v691 狀態 | 備注 |
|-----|------|-----------|------|
| P1 marginAudit SQL | P1 | ❌ 未修復 | v691 尚未部署（bundle hash 未變）|
| AI 對話歷史持久化 | P2 | ✅ **已修復** | 跨 navigate 歷史正確保留 |
| Newsletter raw JSON | P2 | ❌ 未修復 | 電子報欄位仍吐原始 JSON |

**結論：** v691 尚未上線到 prod。AI 對話歷史已修復（可能是服務端 fix 不需要重新部署 bundle，或 PR 已 merge 但前端 bundle 尚未重新部署）。P1 和 newsletter JSON 兩項待 bundle 更新後重驗。
