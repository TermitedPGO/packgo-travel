...
### 5.1 `customerConversationThread` query(adminCustomers.ts:1400)

加可選 `orderId`:
- `orderId` 有 → `customerInteractions` 多一個 `AND customOrderId = orderId`;**inquiries / inquiryMessages 這兩源在專案視圖隱藏**(它們是下單前的首次接觸,不屬於某一單)。
- `orderId` 無(未分類)→ `customerInteractions` 多 `AND customOrderId IS NULL`;inquiries / inquiryMessages 照舊全顯示。

語意:**首次詢問永遠在未分類;成交後的 Gmail 往來可被指派到專案。** 跨客人 leakage 規則(該 query 既有的 verified-email / profileId resolution)完全不放鬆,只多一個 orderId AND。
