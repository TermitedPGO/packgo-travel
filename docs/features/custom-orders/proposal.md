...
- status enum:`draft → quoted → deposit_paid → paid → confirmed → departed → completed / cancelled`
- quoteId(→ aiQuotes,引用報價 PDF)
...
## 六、待你拍板的決策(進 Stage 2 前必填)

1. 生命週期狀態:上面 `draft→quoted→deposit_paid→paid→confirmed→departed→completed` 對嗎?要不要「waitlist / 等供應商確認」這種中間態(像 bookings.supplierStatus)?
2. 訂金:固定 %(套裝是 20%)還是每單自訂?訂金進 Trust 的記錄要不要這個系統管,還是只記「已收訂金」、實際 Trust 對帳仍走銀行/會計?
3. 付款:Square 連結維持「你手動建、貼進來」?還是這階段要接 Square API 自動產生付款連結(較大)?
4. 確認書:內容要放什麼(行程摘要、金額、付款狀態、出發日、PACK&GO 黑白品牌)?用哪個 skill / template 產(packgo-quote 風格?新做?)?要不要客人可線上看(像 quote pdfHtml fallback)?
5. 報價:訂單的「報價」是引用你 skill 出好的 PDF(你上傳/貼 URL),還是要在系統裡觸發報價產生?(建議前者 —— 報價留人力)
6. 與 `bookings` 關係:訂製單要不要也開一筆 bookings(供 seat/供應商流程共用),還是 customOrders 完全獨立、bookings 只給套裝?
7. 範圍邊界:這批先做到哪?(建議:order 表 + 報價/催款/確認三動作的「送+狀態機」,recognition/會計整合列為下一階段。)

---

## 七、非目標(這批不做)

- Square API 自動金流 / 自動對帳(除非 #3 選自動)。
- 自動產生報價內容/價格(報價留人力)。
