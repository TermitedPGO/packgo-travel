...
- **塊D**:兩筆 prod 資料寫入,ship 後 ops(ORD-2026-0010 category flight→quote;金宥卡 2760048 note append,見 T6)。
- **塊B/C**列下一 tranche:塊B 需 migration 0112(caseLearnings sourceOrderId 改 nullable + sourceFolder 冪等欄)+ md 踩坑解析 + LLM 去識別化,不宜長 session 尾端趕 prod schema 變更;塊C 加一條案件對話檔進場端點餵既有 chatLogImport(未來日期一律不建互動)。

**驗證**:tsc 0 錯;全套 `293 passed | 11 skipped (304)` / `4346 passed | 90 skipped (4436)`;i18n 100%。

**狀態**:塊A 已 commit + push origin/main(`432daef`),未 ship。
