...
## 附錄 A:gmail-poll 清理腳本原文(稽核留檔)

本批唯一的 prod 寫操作。腳本內建硬性守門(重跑 getFailed 驗證 == 36 筆、全部 gmail-poll-tick、
finishedOn 全落在 {2026-05-27, 2026-06-17},任一不符即零移除中止),相符才逐筆 `job.remove()`,
只動 gmail-poll 的 failed 集合。逐字如下:
...
(async () => {
  ...
  const conn = new IORedis(process.env.UPSTASH_REDIS_URL, {
    ...
    tls: { rejectUnauthorized: false }, lazyConnect: false,
  });
  const q = new Queue('gmail-poll', { connection: conn });
  const out = (o) => console.log('__RESULT__' + JSON.stringify(o) + '__END__');
  try {
    const beforeCount = await q.getFailedCount();
    const jobs = await q.getFailed(0, 499);
    const dateOf = (ms) => (typeof ms === 'number' ? new Date(ms).toISOString().slice(0, 10) : null);
    const ALLOWED_DATES = new Set(['2026-05-27', '2026-06-17']);

    // ── 硬性守門:全部條件相符才 remove,任一不符零移除中止 ──
    const reasons = [];
    if (beforeCount !== 36) reasons.push('getFailedCount=' + beforeCount + ' !== 36');
    if (jobs.length !== 36) reasons.push('fetched=' + jobs.length + ' !== 36');
    const badName = jobs.filter((j) => j.name !== 'gmail-poll-tick');
    if (badName.length) reasons.push(badName.length + ' jobs name !== gmail-poll-tick (e.g. id=' + badName[0].id + ' name=' + badName[0].name + ')');
    const badDate = jobs.filter((j) => !ALLOWED_DATES.has(dateOf(j.finishedOn)));
    if (badDate.length) reasons.push(badDate.length + ' jobs finishedOn not in {5/27,6/17} (e.g. id=' + badDate[0].id + ' date=' + dateOf(badDate[0].finishedOn) + ')');

    const byDate = {};
    for (const j of jobs) { const d = dateOf(j.finishedOn); byDate[d] = (byDate[d] || 0) + 1; }

    if (reasons.length) {
      out({ action: 'ABORT_MISMATCH', beforeCount, fetched: jobs.length, byDate, reasons });
      return;
    }

    // 相符 → 逐筆 remove(只動 gmail-poll 的 job)
    const removed = [];
    const failedRemove = [];
    for (const j of jobs) {
      try { await j.remove(); removed.push(j.id); }
      catch (e) { failedRemove.push({ id: j.id, err: (e && e.message || String(e)).slice(0, 120) }); }
    }
    const afterCount = await q.getFailedCount();
    out({ action: 'REMOVED', beforeCount, afterCount, removedN: removed.length, failedRemoveN: failedRemove.length, byDate, failedRemove: failedRemove.slice(0, 5) });
  } catch (e) {
    out({ action: 'ERROR', err: (e && e.message || String(e)).slice(0, 200) });
  } finally {
...
呼叫方式(blob 走 stdin,DATABASE_URL/UPSTASH_REDIS_URL 由機上 env 提供,不進命令列):

```
timeout 120 flyctl ssh console -a packgo-travel -C "sh -lc 'node'" < gmail_cleanup.cjs
```

實際輸出(守門通過 → 執行):

```json
{ "action": "REMOVED", "beforeCount": 36, "afterCount": 0, "removedN": 36, "failedRemoveN": 0,
  "byDate": { "2026-06-17": 33, "2026-05-27": 3 }, "failedRemove": [] }
```

獨立唯讀複查(另一支腳本,只讀不寫):`{"failedCount":0,"counts":{"failed":0,"completed":100,"active":0,"waiting":0,"delayed":1,"paused":0}}`。
