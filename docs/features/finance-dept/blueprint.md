...
省稅鏡頭(2026-07-09 Jeff 裁示納入):以現有 financeAdvisor(唯讀 agent)為底擴充,建議卡類型 —
1. 抓漏:掃 bankTransactions 找未分類/誤分類但可能可扣的支出(軟體訂閱、電話網路業務比例、商務差旅、home office),逐筆列出候選請 Jeff 確認分類。
2. 時點:年底加速支出/遞延收入提醒;Trust 認列時點本身是合法稅務時點(未出發訂金遞延不計本年稅,D 細節層已畫)。
3. 額度:自雇退休金(Solo 401k / SEP IRA)供款上限提示(按真實淨利算)、自雇健保費扣除、QBI §199A 20% 適用性。
4. 結構:淨利跨門檻時 S-corp election 損益兩算(自雇稅節省 vs payroll 成本),備料給 CPA 判。
5. 合規防罰:季度預估稅 1040-ES 提醒(D 已畫待建)、1099-NEC 供應商付款 ≥$600 追蹤(D 已畫)。
邊界:每張卡附數據來源 + IRS 出處(publication/行號),固定標註「與 CPA 確認後執行」;AI 不報稅、不代選 election、不動錢,最終判斷歸 CPA。分類建議只能出自 Schedule C 鎖死枚舉(Jeff 2026-07-09 裁決:嚴格 IRS 分類)。
