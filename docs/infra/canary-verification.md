# Canary 驗證雙腳本(9a / 9b)
> 重要（2026-07-23 Jeff 裁定）：本文件描述的兩支腳本，**不再是人工驗證的做法**。
> 經四輪對抗審查，自寫腳本在「人為貼錯連線字串」時反覆出現把明文密碼印到畫面的問題
> （最後一次是密碼落到主機名位置時仍會被印出並送去 DNS 解析）。Jeff 已裁定改用系統
> 內建的標準 mysql 客戶端搭配一份 SQL 檢核清單，密碼由客戶端以隱藏方式當場詢問，
> 從頭到尾不經過任何自寫程式。
>
> 人工驗證請改依：`~/Desktop/PACKGO_AI交流/網站專案/DB加固_後台操作指南.md` 步驟 9。
>
> 本文件與兩支腳本保留供未來自動化使用（由設定檔提供、非人工輸入的連線字串），
> **不得指示任何人手動貼上含真實密碼的連線字串執行**。
