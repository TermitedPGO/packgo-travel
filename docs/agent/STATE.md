...
- 線三重建前置完工待 ship v809:兩審計(UV=GO;Lion=小修可通)+ R1 四修 + R2(Unsplash 署名 migration 0115 + 匯率後衛)+ R3(registry 同步 + catalog-rebuild script-token 端點)+ R4(必付費用真修:必付自費分流,Jeff 點出的全量硬前置,零 migration)。指揮三輪驗收 PASS,已併 main。ship 後指揮跑 dryRun(skipSync:false 必帶,enrich 自癒含蓋)→ 25 團試批 → Jeff 看貨架才全量;UNSPLASH_ACCESS_KEY 缺則試批無圖(Jeff 辦 key)。
