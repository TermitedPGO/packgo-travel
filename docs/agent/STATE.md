...
- 線三重建前置完工待 ship v809:兩審計(UV=GO 價格紅線清白;Lion=小修可通+destinationCountry 真蟲)+ R1 四修 + R2(Unsplash 署名合規 migration 0115 + 匯率後衛)+ R3(registry 同步 + /api/admin/catalog-rebuild script-token 端點,dryRun 預設/promote 需 confirm)。指揮兩輪驗收 PASS,已併 main。ship 後指揮跑 dryRun → 25 團試批 → Jeff 看貨架才全量。
