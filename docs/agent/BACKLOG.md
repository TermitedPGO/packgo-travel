...
## 外部 AI 意見採納批(2026-07-11 第一輪,詳 docs/agent/external-exchange-round1.md)

1. [預授權] DB 權限隔離:應用/AI 角色永久禁 DDL,migration 走獨立短效升權角色(6/17 事故的預防層,現只有偵測層)。
2. [Jeff+預授權] 備份還原演練:Jeff 查 TiDB 保留期(原待辦升級)→ 指揮設計演練(還原到隔離環境+訂單/商品/信託/銀行重新勾稽驗證)。
3. [預授權] 信託差異 $10.4k 六桶拆解查核(時間差/未配對/重複/退款/錯誤提領/真短缺),取代原「查核」籠統項。
4. [Jeff] CPA 問題升級一頁式判斷矩陣:各商品情境 × 四時點(現金移動/法律可提領/管理認列/稅務認列);§17550.15 提領條件不只出發日,現行為內部保守政策。
5. [預授權] 結帳合規:付款前揭露(行程/取消政策/總額/必付/信託保障)+ 客戶同意版本存證(DOJ 要求)。
6. [預授權] 郵件自動寄送制度化:錯誤後果分級(價格/餘位/退款/簽證/新承諾預設人審)、新類型影子模式、10/25/50/100 放量、5% 抽樣、超標降級;逐類 300 封零重大錯誤才放權。
7. [預授權] 漏斗量測:詳情頁→詢問→付款轉換率、每單 Jeff 分鐘數、待決策數與最老卡片天數,上架同批埋。
8. [裁決門] 供給策略分歧:外部主張先 30-50 團驗需求,指揮主張全量+首頁精選策展層並行 —— Jeff 裁。
