...
export const appRouter = router({
  ...
  tours: router({
    ...
    // Round 55 Diag-B: Temporary — remove after use
    diagnoseAgents: adminProcedure
      .mutation(async () => {
        ...
        try {
          ...
          const resp = await Promise.race([
            invokeLLM({ messages: [{ role: 'user', content: '\u56de\u7b54\u300cOK\u300d\u5c31\u597d' }] }),
            new Promise<never>((_, reject) => setTimeout(() => reject(new Error('LLM timeout 30s')), 30000)),
          ]);
          const content = (resp as any)?.choices?.[0]?.message?.content ?? '';
          results.llm = { ok: true, response: content.slice(0, 50), ms: Date.now() - start };
        } catch (err: any) {
        ...
        try {
          const { ContentAnalyzerAgent } = await import('./agents/contentAnalyzerAgent');
          const agent = new ContentAnalyzerAgent();
          const start = Date.now();
          const testData = {
            basicInfo: { title: '\u6e2c\u8a66\u884c\u7a0b\u4e09\u65e5', subtitle: '', description: '\u9019\u662f\u6e2c\u8a66', productCode: '' },
            location: { destinationCountry: '\u53f0\u7063', destinationCity: '\u82b1\u84ee' },
            duration: { days: 3, nights: 2 },
            pricing: { price: 15000, basePrice: 15000, currency: 'TWD' },
            highlights: [], dailyItinerary: [], includes: [], excludes: [],
            accommodation: [], hotels: [], meals: [], flights: [], notices: [],
            images: [], rawContent: '\u82b1\u84ee\u4e09\u65e5\u904a\uff0c\u542b\u592a\u9b6f\u95a3\u3001\u4e03\u661f\u6f6d\u3001\u9be4\u9b5a\u6f6d\u3002\u50f9\u683c NT$15,000\u3002',
            renderedHtml: '', sourceUrl: '', isPdfSource: false,
          };
          const result = await Promise.race([
            agent.execute(testData),
            new Promise<never>((_, reject) => setTimeout(() => reject(new Error('ContentAnalyzer timeout 60s')), 60000)),
          ]);
          results.contentAnalyzer = { ok: (result as any).success, ms: Date.now() - start };
        } catch (err: any) {
          results.contentAnalyzer = { ok: false, error: err.message };
        }

        // Test 2: ColorThemeAgent
        try {
          const { ColorThemeAgent } = await import('./agents/colorThemeAgent');
          const agent = new ColorThemeAgent();
          const start = Date.now();
          const result = await Promise.race([
            agent.execute('\u53f0\u7063', '\u82b1\u84ee'),
            new Promise<never>((_, reject) => setTimeout(() => reject(new Error('ColorTheme timeout 30s')), 30000)),
          ]);
          results.colorTheme = { ok: (result as any).success, ms: Date.now() - start };
        } catch (err: any) {
          results.colorTheme = { ok: false, error: err.message };
        }

        // Test 3: ItineraryUnifiedAgent
        try {
          const { ItineraryUnifiedAgent } = await import('./agents/itineraryUnifiedAgent');
          const agent = new ItineraryUnifiedAgent();
          const start = Date.now();
          const testData = {
            basicInfo: { title: '\u82b1\u84ee\u4e09\u65e5' },
            location: { destinationCountry: '\u53f0\u7063', destinationCity: '\u82b1\u84ee' },
            duration: { days: 3, nights: 2 },
            pricing: { price: 15000 },
            rawContent: '\u7b2c\u4e00\u5929\uff1a\u592a\u9b6f\u95a3\u3002\u7b2c\u4e8c\u5929\uff1a\u4e03\u661f\u6f6d\u3002\u7b2c\u4e09\u5929\uff1a\u9be4\u9b5a\u6f6d\u3002',
            dailyItinerary: [
              { day: 1, title: '\u592a\u9b6f\u95a3', description: '\u592a\u9b6f\u95a3\u570b\u5bb6\u516c\u5712' },
              { day: 2, title: '\u4e03\u661f\u6f6d', description: '\u4e03\u661f\u6f6d\u6d77\u5cb8' },
              { day: 3, title: '\u9be4\u9b5a\u6f6d', description: '\u9be4\u9b5a\u6f6d\u98a8\u666f\u5340' },
            ],
          };
          const result = await Promise.race([
            agent.execute(testData),
            new Promise<never>((_, reject) => setTimeout(() => reject(new Error('Itinerary timeout 60s')), 60000)),
          ]);
          results.itinerary = { ok: (result as any).success, ms: Date.now() - start };
        } catch (err: any) {
          results.itinerary = { ok: false, error: err.message };
        }
