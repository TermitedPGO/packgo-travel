/**
 * trustTransferDetection 純函式測試 — F2 塊B(2026-07-10)。
 *
 * 覆蓋:
 ...
 *     的 isRecognitionDue)。
 *   - run_group 規則:同 recognitionRunId 群組加總全等(對應每日認列後
 *     notifyOwner 叫 Jeff 一次轉當日總額的實際操作流)。
 */
...
const TRUST = new Set([10]); // linkedAccountId 10 = trust,其他 = operating
...
describe("matchPairsToDeferrals — 配對 → 遞延列回填", () => {
  it("single 規則:恰好一列已認列同額 → 回填,金額/流水 id/曆日釘死", () => {
    const backfills = matchPairsToDeferrals([pair()], [row({ id: 501 })]);
  ...
  it("紅(§17550 #3):未認列列金額全等也絕不回填", () => {
    const backfills = matchPairsToDeferrals([pair()], [row({ id: 501, recognizedAt: null })]);
    expect(backfills).toEqual([]);
  });
  ...
  it("歧義:兩列同額都 eligible → 跳過留給人", () => {
    const backfills = matchPairsToDeferrals([pair()], [row({ id: 501 }), row({ id: 502 })]);
    expect(backfills).toEqual([]);
  });

  it("帳戶不符(遞延列掛別的 trust 帳)→ 不回填;哨兵列 linkedAccountId=0 天然不中", () => {
    expect(
      matchPairsToDeferrals([pair()], [row({ id: 501, linkedAccountId: 99 })]),
    ).toEqual([]);
    expect(
      matchPairsToDeferrals([pair()], [row({ id: 502, linkedAccountId: 0 })]),
    ).toEqual([]);
  });

  it("run_group 規則:同 recognitionRunId 兩列加總全等 → 整組回填(每日轉總額流)", () => {
    const backfills = matchPairsToDeferrals(
    ...
    expect(backfills.map((b) => b.deferredId).sort()).toEqual([601, 602]);
    expect(backfills.every((b) => b.rule === "run_group")).toBe(true);
    expect(backfills.every((b) => b.transferBankTransactionId === 900)).toBe(true);
  });

  it("run_group 歧義:兩組不同 runId 加總都全等 → 跳過", () => {
    const backfills = matchPairsToDeferrals(
