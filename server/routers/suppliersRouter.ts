...
export const suppliersRouter = router({
  ...
  /**
   * Deactivate all supplier-imported tours that haven't been through
   * the full AI masterAgent rewrite pipeline. Heuristic: tours with
   * a supplier sourceUrl (liontravel/uvbookings) AND no poeticSubtitle
   * were never AI-rewritten — they only have hydrated supplier data.
   *
   * The 134 AI-enriched tours DO have poeticSubtitle set (generated by
   * contentAnalyzerAgent) and will be preserved.
   *
   * 2026-05-27: Jeff's rule — unwritten tours should not be on the
   * public tour listing. They go to draft (inactive) until rewritten.
   */
  deactivateUnrewritten: adminProcedure
    .input(
      z.object({
        dryRun: z.boolean().default(true),
      }),
    ...
    .mutation(async ({ input }) => {
      ...
      const rows = await db2
        ...
        .where(
          and(
            ...
            or(
              isNull(toursTable.poeticSubtitle),
              eq(toursTable.poeticSubtitle, ""),
            ),
          ),
        );
      ...
      await db2
        ...
        .where(condition);
      return { dryRun: false, deactivated: rows.length };
    }),
