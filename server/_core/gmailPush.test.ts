...
describe("fetchHistoryPage — labelAdded 發現 (三宇宙一致, 對抗審查修正 1)", () => {
  it("labelAdded 發現: a message moved INTO the inbox later (labelAdded INBOX) is discovered", async () => {
    ...
    // maxSeenEventId / labelEventId (Codex 17 輪 §四.3) = each message's OWN history record
    // id, NOT the page boundary/snapshot (700). A plain new message carries no label event
    // (labelEventId null); the moved-into-inbox one carries the label-only requeue-trigger id.
    expect(byId["fresh"].maxSeenEventId).toBe("601");
    expect(byId["fresh"].labelEventId).toBeNull();
    expect(byId["moved-in"].maxSeenEventId).toBe("602");
    expect(byId["moved-in"].labelEventId).toBe("602");
    expect(out.boundaryHistoryId).not.toBe(byId["moved-in"].maxSeenEventId); // event id ≠ boundary
  });
  ...
  it("the same message in messagesAdded AND labelsAdded dedups to one discovery", async () => {
    ...
    // both events share the ONE record 604 → both watermarks are 604.
    expect(out.messages[0].maxSeenEventId).toBe("604");
    expect(out.messages[0].labelEventId).toBe("604");
  });

  it("labelsAdded BEFORE messagesAdded (same id): kind stays label, but the two watermarks split (Codex 17 §四.3)", async () => {
    // order-independence of the upgrade: the label record comes first, then the
    // messageAdded record re-sees the id — eventKind must NOT be downgraded. The two
    // per-event watermarks are tracked SEPARATELY (Codex 17 輪 §四.3): maxSeenEventId is the
    // MAX over ALL records; labelEventId is the MAX over ONLY the labelAdded(INBOX) records.
    // Sharing one max (old bug) would let the label-kind discovery carry the 606 messagesAdded
    // id as its requeue-trigger id — the exact "label kind 攜帶非 label 事件 id" hazard.
    const gmail = gmailStub([
      {
        historyId: "703",
        history: [
    ...
    expect(out.messages[0].eventKind).toBe("label_added_inbox");
    expect(out.messages[0].maxSeenEventId).toBe("606"); // MAX over messagesAdded ∪ labelsAdded
    expect(out.messages[0].labelEventId).toBe("605"); // label-only max — NEVER the 606 message id
  });
  ...
  it("大數 non-final page: boundary = last history[].id, and the far-larger top-level snapshot is NOT stored", async () => {
    ...
    // per-event ids (>2^53): each message keeps its OWN record id, never the boundary
    // or the far-larger top-level snapshot. 'a' is messagesAdded (no label event); 'b' is
    // labelAdded(INBOX) so it carries a label-only requeue-trigger id too.
    const byId = Object.fromEntries(out.messages.map((m) => [m.id, m]));
    expect(byId["a"].maxSeenEventId).toBe("9007199254740611");
    expect(byId["a"].labelEventId).toBeNull();
    expect(byId["b"].maxSeenEventId).toBe("9007199254740613");
    expect(byId["b"].labelEventId).toBe("9007199254740613");
    expect(byId["a"].maxSeenEventId).not.toBe("9007199254749999");
  });
