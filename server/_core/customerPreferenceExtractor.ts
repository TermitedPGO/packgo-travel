...
export async function analyzeOrderAiUnderstanding(
  ...
  const userPrompt = [
    // 日期 grounding(2026-07-02):素材裡客人只寫月/日沒寫年份時,模型會腦補
    // 過去年份(真實案例:2026 年來信講 12/19 被寫成 2024/12/19)。鐵律「嚴禁
    // 推測」不變 — 這行只管「年份怎麼讀」:要嘛照原文不加年份,要加就推最近
    // 的未來,絕不編過去年份。
    `【今天日期(美西)】${DAY_LA.format(new Date())}。素材裡沒寫年份的日期,照原文寫或按今天日期推最近的未來年份,絕不要編成過去的年份。`,
    "",
    "【專案基本欄位(系統真相,搬運勿改)】",
    factLines.join("\n"),
    "",
