...
export const WHITELIST: WhitelistEntry[] = [
  { source: "server/_core/healthCheck.ts:178", reason: "SELECT 1 是純健康探測(liveness ping),不含任何表/欄位,不對應任何 schema 查詢,EXPLAIN 對它無意義(MySQL/TiDB 對 SELECT 1 的 EXPLAIN 只會顯示 'No tables used',沒有計畫可彩排)。" },
  { source: "server/_core/schemaContract.ts:97", reason: "schema 契約探測:查 information_schema.tables 確認 REQUIRED_TABLES 存在(健康檢查/煙霧用),對 metadata 表 EXPLAIN 無彩排意義,且比對邏輯由 schemaContract.test.ts 純函式單測覆蓋。" },
