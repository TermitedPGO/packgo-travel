...
export async function handleStripeWebhook(req: Request, res: Response) {
  ...
  try {
    switch (event.type) {
      ...
      // 2026-07-25 Jeff 裁定:不應有付費會員機制。customer.subscription.*
      // 與 trial_will_end 的處理已整組移除(訂閱結帳端點同日自
      // membership router 移除,不會再有訂閱事件;若 Stripe 後台仍送來,
      // 走下方 default 記 log 即可)。依據見 server/routers/membership.ts 檔頭。
...
async function handleCheckoutSessionCompleted(session: Stripe.Checkout.Session) {
  ...
  // 2026-07-25:mode === "subscription" 的 fallback 已移除(無付費會員機制)。
  // 訂閱型 session 不再產生;真的收到就落到下面的 booking/visa 分支外,不動任何 tier。
  ...
  // Visa branch short-circuits before this block — it has its own atomicity
  // story (module 2.5)。(2026-07-26:subscription branch 已整組移除。)
...
// 2026-07-25:handleSubscriptionUpserted、handleTrialWillEnd、
// handleSubscriptionDeleted 已整組移除(不應有付費會員機制)。
// 歷史實作見 git;恢復付費層前必讀 membership.ts 檔頭的法遵條件。
