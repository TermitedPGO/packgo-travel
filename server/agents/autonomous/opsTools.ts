...
export const WRITE_TOOLS: Anthropic.Tool[] = [
  ...
  },
  {
    ...
    description:
      ...
      "存進客人文件。若這位客人已有待審回信草稿,系統會自動把 PDF 掛上;若還沒有草稿,就先出文件、" +
      "再用回信/跟進草稿工具產生回覆草稿(系統會把這份剛產生的 PDF 掛上),等 Jeff 按確認發送才寄出 —— 你絕不自動寄。" +
      "在檔案真的掛上草稿前,信裡不要寫「附上/附在信裡」等字樣。" +
    ...
    case "generate_customer_document": {
      ...
      try {
        ...
        const draftLine = attached
          ...
          : "目前這位客人沒有待回的草稿卡,PDF 還沒掛到任何一封信上(文件已在客人文件 tab)。" +
            "正確順序:接著用回信/跟進草稿工具產生一封回覆草稿,系統會自動把這份剛產生的 PDF 掛上,再讓 Jeff 確認寄出。" +
            "在草稿真的掛上檔案前,信裡不要寫「附上/附在信裡/隨信附上」等字樣 —— 沒帶檔案卻這樣寫,寄送會被誠實閘擋下。";
        return {
          success: true,
          kind,
          orderId,
