...
// ── ⑥ 洪水閘(Wave1 Block B, 2026-07-08)— 逐信失敗卡的粒度保留,但一輪失敗 > 5 封時
// 收斂成一張聚合卡,避免刷屏轟炸 Jeff 的收件匣。errorFunnel 的去重機制不適用在這裡,
// 這個閘是獨立實作(faildThisRun 陣列 + 迴圈跑完後才決定貼幾張卡)。
describe("ingestFreshMessages — 洪水閘:>5 封失敗聚合成一張卡,≤5 封仍逐封貼卡", () => {
  it("2 封失敗 → notifyAgentMessage 呼叫兩次,各自是獨立卡(不聚合)", async () => {
    listHistoryMessageIdsMock.mockResolvedValue({
      messageIds: ["m-a", "m-b"],
      latestHistoryId: "410",
      expired: false,
    ...
    listMessagesByIdsMock.mockResolvedValue([
      makeMsg("m-a", "A <a@customerdomain.com>"),
      makeMsg("m-b", "B <b@customerdomain.com>"),
    ]);
    detectReceiptMock.mockReturnValue({ isReceipt: false });
    runInquiryAgentMock.mockRejectedValue(new Error("boom"));

    const result = await runGmailPipelineForMessageIds(7, "411");

    expect(result.totalFailed).toBe(2);
    expect(notifyAgentMessageMock).toHaveBeenCalledTimes(2);
    const titles = notifyAgentMessageMock.mock.calls.map(
      (c) => (c[0] as { title: string }).title,
    );
    expect(titles[0]).toContain("a@customerdomain.com");
    expect(titles[1]).toContain("b@customerdomain.com");
    // 不是聚合卡:標題是「一封客人來信處理失敗」的單封格式,不含「已聚合」。
    expect(titles.every((t) => !t.includes("已聚合"))).toBe(true);
  });

  it("剛好 5 封失敗 → 仍走個別路徑(5 次呼叫,不聚合,邊界是 > 5 不是 >= 5)", async () => {
    ...
    listMessagesByIdsMock.mockResolvedValue(ids.map((id) => makeMsg(id, `${id}@customerdomain.com`)));
    detectReceiptMock.mockReturnValue({ isReceipt: false });
    runInquiryAgentMock.mockRejectedValue(new Error("boom"));

    const result = await runGmailPipelineForMessageIds(7, "421");

    expect(result.totalFailed).toBe(5);
    expect(notifyAgentMessageMock).toHaveBeenCalledTimes(5);
    const titles = notifyAgentMessageMock.mock.calls.map(
      (c) => (c[0] as { title: string }).title,
    );
    expect(titles.every((t) => !t.includes("已聚合"))).toBe(true);
  ...
  it("6 封失敗 → notifyAgentMessage 恰好呼叫一次,是聚合卡且列出全部 6 個 msgId", async () => {
    ...
    const result = await runGmailPipelineForMessageIds(7, "431");
    ...
    expect(card.messageType).toBe("alert");
    expect(card.priority).toBe("high");
    expect(card.title).toContain("已聚合");
    expect(card.title).toContain("6");
    for (const id of ids) {
      expect(card.body).toContain(id);
    }
  });
});
