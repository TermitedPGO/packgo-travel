...
export default function ToursTab() {
  ...
  const submitAsyncGenerationMutation = trpc.tours.submitAsyncGeneration.useMutation({
    onSuccess: (result) => {
      ...
      // v70: opportunistically request browser notification permission so the
      // completion toast can also fire as a push when admin is on another tab.
      // First-time prompt only — once decided, the browser remembers.
      try {
        if (
  ...
  useEffect(() => {
    ...
    if (generationStatus.status === "completed") {
      ...
      if (generationStatus.result?.success) {
        ...
        const tourIdLabel = tourId ? t('toursTab.tourIdLabel').replace('{id}', String(tourId)) : undefined;
        ...
        toast.success(
          t('toursTab.generationSuccess'),
          {
            ...
            duration: 5000,
            // v70: clickable toast — admin can jump straight to the new tour
            action: tourId
              ? {
        ...
        // v70: also fire a browser push notification when the admin tab is
        // backgrounded. Without this, generations completed while admin is in
        // another tab silently disappear and the new tour becomes a "ghost".
        try {
          if (
            typeof window !== 'undefined' &&
            'Notification' in window &&
            ...
            const n = new Notification(t('toursTab.generationSuccess'), {
              body: tourIdLabel || '',
              icon: '/favicon.ico',
              tag: `tour-gen-${tourId || 'unknown'}`, // de-dupe if multiple fire
            });
            n.onclick = () => {
              window.focus();
        ...
        } catch { /* notification API not available — toast was already shown */ }
      }
    }
