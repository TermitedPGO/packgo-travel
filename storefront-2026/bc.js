...
function bcFooter() {
  ...
  return `<footer class="bc-site-footer"><div class="bc-footer-inner"><div class="bc-footer-top">
    <div class="bc-footer-brand"><img class="bc-footer-wordmark" src="${BC_BRAND.wordmark}" alt="PACK&GO ${L("旅行社","Travel Agency","Agencia de viajes")}"/><p>${L("用旅圖看見嚮往　用清楚的班期與價格安心出發　需要判斷的地方始終有人接手", "See where you want to go through images then depart with clear dates prices and personal support", "Vea el destino en imágenes y viaje con fechas precios y apoyo claros")}</p><div class="bc-footer-social"><small>${L("旅行靈感與出發提醒","TRAVEL NOTES & DEPARTURE UPDATES","IDEAS Y AVISOS DE VIAJE")}</small><div>${social("Instagram", instagramMark)}${social("Facebook", facebookMark)}</div><em>${L("社群帳號整理中","Social accounts coming soon","Demo · enlaces oficiales pendientes")}</em></div></div>
    ${col(L("尋找旅程","FIND A JOURNEY","ENCONTRAR VIAJE"),[["tours",L("團體行程","Group journeys","Circuitos")],["custom",L("客製旅行","Custom travel","Viaje a medida")],["about",L("認識 PACK&GO","Meet PACK&GO","Conocer PACK&GO")]])}
    ...
    ${col(L("聯絡我們","CONTACT US","CONTACTO"),[["contact",L("聯絡與支援","Contact and support","Contacto y soporte")],["help",L("幫助中心","Help center","Centro de ayuda")],["about",L("關於 PACK&GO","About PACK&GO","Sobre PACK&GO")]])}
  </div><div class="bc-footer-bottom"><nav class="bc-footer-utility-links" aria-label="${L("頁尾導覽","Footer navigation","Navegación del pie")}"><button data-view="help">${L("幫助中心","Help center","Centro de ayuda")}</button></nav><address>PACK&GO · 39055 Cedar Blvd #126, Newark, CA 94560<br>+1 (510) 634-2307 · support@packgoplay.com</address><div><span data-keep-punctuation>California Seller of Travel CST #2166984 · Newark Business License #115594</span><br><span>${L("TCRF 適用條件以付款前揭露與加州法律為準。", "TCRF eligibility follows pre-payment disclosure and California law.", "La elegibilidad TCRF depende de la divulgación previa y la ley de California. Prototipo sin transacciones reales.")}</span></div></div></div></footer>`;
}
...
function bcTourCard(t) {
  ...
  return `<article class="bc-tour-card" data-tour-card data-status="${t.formed?"confirmed":"collecting"}" data-air="${t.air?"included":"separate"}" data-search="${searchable}">
    ...
    <div class="bc-tour-body"><div class="bc-tour-heading"><div><span class="bc-tour-code">${t.code} · ${bcTxt(t.destination)}</span><h2>${bcTxt(t.title)}</h2></div></div>
      ...
      <div class="bc-tour-bottom"><div class="bc-tour-price"><small>${L("每人起","PER PERSON","POR PERSONA")}</small><strong>${money(t.price)}</strong></div><button class="btn small" data-tour="${t.id}">${L("查看詳情","View details","Ver detalles")}</button></div>
    </div></article>`;
}
...
function helpBC() {
  return `<main>${bcPageIntro("HELP & POLICIES",L("先找到答案，再決定是否需要真人。","Find the answer before you need a person.","Encuentre la respuesta antes de necesitar ayuda."),L("訂位、付款、旅客資料、機票、中國簽證與取消都有清楚入口；高風險例外再建立支援案件。","Booking, payment, travelers, flights, China visas and cancellation each have a clear path; high-risk exceptions open a support case.","Reserva, pago, viajeros, vuelos, visado chino y cancelación tienen rutas claras; las excepciones abren soporte."),BC_IMG.support)}<section class="bc-shell section-tight"><div class="bc-support-grid"><button class="bc-support-card" data-view="contact"><span><p class="eyebrow">BOOKING</p><h3>${L("訂位與訂單進度","Booking and order progress","Reserva y progreso")}</h3><p>${L("從我的訂單查看狀態、下一步與費用。","See state, next action and cost in My Orders.","Vea estado, siguiente paso y costo en Mis pedidos.")}</p></span><b>${L("聯絡我們查詢 →","Ask us →","Consúltenos →")}</b></button><button class="bc-support-card" data-view="contact"><span><p class="eyebrow">PAYMENT</p><h3>${L("付款與收據","Payments and receipts","Pagos y recibos")}</h3><p>${L("確認付款對象、狀態、到期日與收據。","Confirm payee, state, due date and receipt.","Confirme quién cobra, estado, vencimiento y recibo.")}</p></span><b>${L("聯絡我們 →","Contact us →","Contáctenos →")}</b></button><button class="bc-support-card" data-view="contact"><span><p class="eyebrow">EXCEPTIONS</p><h3>${L("建立支援案件","Open a support case","Abrir soporte")}</h3><p>${L("只有需要真人判斷的例外才進入支援。","Only exceptions needing judgment enter support.","Solo las excepciones que requieren juicio entran en soporte.")}</p></span><b>${L("聯絡與支援 →","Contact & support →","Contacto →")}</b></button></div></section></main>`;
}
...
function bcHeader() {
  ...
  // 2026-07-29:會員區(登入、訂單、付款)還在重建,尚未對客開放。在那之前頁首
  // 不顯示登入入口 —— 給一個按下去只會說「還在整理」的按鈕,比不給更糟。
  // 會員區接好之後把 SHOW_MEMBER_ENTRY 改回 true 即可。
  const SHOW_MEMBER_ENTRY = false;
...
function bcGuidedFlow(id) {
  ...
  if (stepIndex >= flow.steps.length) {
    ...
    const primaryAction = id === "vip" ? bcVipFitAction(flowState.answers) : id === "china" ? bcChinaVisaAction(flowState.answers) : `<button class="btn" data-enquiry-open data-enquiry-flow="${id}">${L("把答案送給我們","Send this to us","Enviárnoslo")}</button>`;
    const completionNote = L("以上答案還沒有送出。按下方按鈕把它們一起寄給我們，我們會直接回覆你；不會自動下訂或扣款。","Your answers have not been sent yet. Send them to us with the button below and we will reply personally. Nothing is booked or charged automatically.","Sus respuestas aún no se han enviado. Envíelas con el botón y le responderemos; no se reserva ni se cobra nada.");
    return `<section class="bc-guided-flow complete"><div class="bc-guided-main"><p class="eyebrow">${flow.eyebrow}</p><div id="bc-flow-result" tabindex="-1" class="bc-guide-result-mark">✓</div>${result}<p>${completionNote}</p><div class="bc-guide-result-actions">${primaryAction}<button class="btn ghost" data-flow-reset data-flow="${id}">${L("重新回答","Start again","Empezar de nuevo")}</button></div></div><aside class="bc-guided-summary"><p class="eyebrow">${L("你的需求摘要","YOUR BRIEF","SU RESUMEN")}</p>${summary}</aside></section>`;
  }
...
function transferBC() {
  return bcGuidedServicePage({
    ...
    briefCopy:L("付款前顯示車型、等待時間、超時費與取消期限。","Show vehicle, wait time, overtime and cancellation before payment. No vehicle is dispatched.","Muestre vehículo, espera, tiempo extra y cancelación. No se despacha vehículo."),
    position:"center 58%",
    facts:[[L("車型與容量","Vehicle and capacity","Vehículo y capacidad"),L("人數、行李與特殊設備一起計算。","Guests, bags and equipment together.","Viajeros, maletas y equipo juntos.")],[L("免費等待","Included wait","Espera incluida"),L("等待時間依接送地點與供應商而定，訂購前會明列在報價上。","Included wait time depends on pickup point and supplier; it is stated on your quote before you book.","El tiempo de espera depende del punto de recogida y del proveedor; se indica en su presupuesto.")],[L("例外由人處理","Human exceptions","Excepciones humanas"),L("輪椅、多站、航班取消與臨時改址。","Wheelchair, multiple stops, cancellation or late address change.","Silla, paradas, cancelación o cambio de dirección.")]]
  });
}
...
function hotelBC() {
  return bcGuidedServicePage({
    ...
    briefCopy:L("這四項固定一起比較。","Compare these four together.","Compare estos cuatro juntos. El prototipo no redirige ni reserva."),
    position:"center 55%",
...
function contactBC() {
  return bcGuidedServicePage({
    ...
    copy:L("告訴我們問題類型、急迫程度與希望解決的事情就好；購買流程本身不需要透過聯絡表單。","Tell us the issue, how urgent it is and the outcome you need. Buying itself never depends on a contact form.","Cuéntenos el problema, la urgencia y el resultado que necesita. La compra no depende del formulario."),
    briefTitle:L("先分流，高風險立即升級","Triage first; escalate risk immediately","Primero clasificar; el riesgo se escala"),
    briefCopy:L("人身安全、重大醫療、護照遺失或行程重大中斷，請直接致電 +1 (510) 634-2307，不要等表單回覆。","For personal safety, serious medical issues, a lost passport or a major disruption, call +1 (510) 634-2307 directly — do not wait for a form reply.","Para seguridad, urgencias médicas, pérdida de pasaporte o interrupciones graves, llame al +1 (510) 634-2307; no espere respuesta del formulario."),
    position:"center 48%",
