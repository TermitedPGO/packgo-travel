...
--
-- Idempotent guards: each ALTER/CREATE wrapped in an information_schema
-- existence check so re-runs don't error.
...
SET @sql := IF(
  NOT EXISTS (
    ...
    WHERE table_schema = DATABASE()
      AND table_name = 'linkedBankAccounts'
  ),
  ...
  'SELECT ''table linkedBankAccounts already exists, skipping'''
);
...
SET @sql := IF(
  ...
  'SELECT ''table bankTransactions already exists, skipping'''
);
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;

-- ─── plaidWebhookEvents (audit trail for incoming webhooks) ─────────────────

SET @sql := IF(
  NOT EXISTS (
    SELECT 1 FROM information_schema.TABLES
    WHERE table_schema = DATABASE()
      AND table_name = 'plaidWebhookEvents'
  ),
  ...
  'SELECT ''table plaidWebhookEvents already exists, skipping'''
);
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;
